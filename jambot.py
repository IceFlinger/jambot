...
class botMain(irc.bot.SingleServerIRCBot):
    ...
    def on_command(self, c, e):
        ...
        if command == "version":
            ...
            self.send_msg(e.target, "Loaded modules:" + modulelist, "jambot")

        elif command == "help" and args:
            if args[0] in self.modulenames:
                function = getattr(self.modulenames, args[0])
                function()

            else:
                self.send_msg(e.target, self.nickname + " help: Module not loaded", "jambot")

        elif command == "help":
            self.send_msg(e.target,
                          self.nickname + " help: Type !modules for a list of modules, and type !help <module name> for help on that module",
                          "jambot")
